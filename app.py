...
# 🌐 ฟังก์ชันดึงข้อมูลพนักงานจาก Google Sheet (แก้ไขการจัดย่อหน้าและเครื่องหมายคำพูดพัง)
...
# 🔗 รายชื่อลิงก์ URL คลังภาพทั้ง 18 แฟ้ม (แก้ไข String Key คลุมอักษร A, B, C)
FOLDER_LINK_MAP = {
    "A": {
...
# ---------------- หน้าสาม: บอร์ดสถิติอิง Material จริง ----------------
    ...
    # แก้ไขตรงนี้: ใส่เครื่องหมายคำพูดครอบสตริงชิ้นงานตัวอย่างเพื่อแก้อาการ Syntax Error ล้มเหลว
    if chart_data.empty:
        chart_data = pd.DataFrame({
    ...
    if not chart_data.empty:
        ...
        # กำหนด State Key ชิ้นงานตัวแปรเลือก
        ...
        # 🛠️ ตรวจจับสัดส่วนรายเอียดตามคอลัมน์ระบบ "Short description" จริงของคุณ
        ...
        # 🍕 1. แผนภูมิวงกลม (Pie Chart) - แยกประเภทสรุปข้อมูลจริงของชิ้นงานที่เลือก
        fig_pie = px.pie(
            pie_df,
            names=pie_names_col,
            values=qty_col,
