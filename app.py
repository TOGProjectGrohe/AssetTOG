...
# --- 2. ดึงข้อมูลจากกูเกิลชีทไฟล์ที่ 1 (ตาราง Asset และ ตารางพนักงาน) ---
@st.cache_data(ttl=1)
def load_base_data():
    def fix_link(url):
    ...
    try:
        # ลิงก์ CSV ไฟล์ข้อมูลทรัพย์สินของคุณ (ตรวจสอบแล้วว่าถูกต้องแม่นยำครับ)
        ...
        # ลิงก์ CSV ไฟล์รายชื่อพนักงานของคุณ (ใส่ลิงก์พนักงานที่ได้จาก Publish to Web ของคุณได้เลยครับ)
        ...
        if 'รูปภาพ GL' in df_emp.columns:
            df_emp['รูปภาพ GL'] = df_emp['รูปภาพ GL'].apply(fix_link)
...
# --- 3. 🎯 ระบบดักจับการสแกนคิวอาร์โค้ดจากเบราว์เซอร์มือถือ ---
query_params = st.query_params

# ดักจับการสแกนรหัสพนักงาน (?emp_id=xxxx)
url_emp = query_params.get("emp_id", None)
# ดักจับการสแกนรหัสทรัพย์สิน (?asset=xxxx)
url_asset = query_params.get("asset", None)

if 'step' not in st.session_state: st.session_state.step = 1
if 'user' not in st.session_state: st.session_state.user = None

# ตรวจสอบสถานะการล็อกอินอัตโนมัติจากการสแกนครั้งแรก
if url_emp and st.session_state.user is None:
    df_emp['รหัสพนักงาน'] = df_emp['รหัสพนักงาน'].astype(str).str.strip()
    match = df_emp[df_emp['รหัสพนักงาน'] == str(url_emp).strip()]
    if not match.empty:
...
# --- 🛑 STEP 1: หน้าสแกนพนักงาน (กรณียังไม่ได้สแกนบัตร) ---
if st.session_state.step == 1:
    ...
    if emp_input or url_emp:
        search_id = str(emp_input if emp_input else url_emp).strip()
        df_emp['รหัสพนักงาน'] = df_emp['รหัสพนักงาน'].astype(str).str.strip()
        match = df_emp[df_emp['รหัสพนักงาน'] == search_id]
        if not match.empty:
            emp_data = match.iloc[0]
            st.session_state.user = emp_data.to_dict()
            st.success("✅ ตรวจพบข้อมูลพนักงาน")
            st.markdown(f'<div class="profile-card"><div><b>ผู้ตรวจ:</b> {emp_data["ชื่อนามสกุล"]}<br><b>ตำแหน่ง:</b> {emp_data["ตำแหน่ง"]} ({emp_data["รหัสพนักงาน"]})</div></div>', unsafe_allow_html=True)
            if emp_data.get('รูปภาพ GL'): st.image(emp_data['รูปภาพ GL'], width=140)
...
# --- 🛑 STEP 2: หน้าสแกนเครื่องจักรหน้างาน + ถ่ายรูปบันทึกผล ---
elif st.session_state.step == 2:
    st.title("🕵️ ตรวจสอบสภาพทรัพย์สินหน้างาน")
    u = st.session_state.user
    st.caption(f"👤 ผู้ลงบันทึก: {u['ชื่อนามสกุล']} ({u['รหัสพนักงาน']})")
    st.markdown('<div class="status-badge">ขั้นตอนที่ 2: บันทึกข้อมูลสภาพจริง</div>', unsafe_allow_html=True)
    
    # ดึงข้อมูลรหัสทรัพย์สินจากการสแกนคิวอาร์โค้ดเครื่องจักรโดยตรง
            ...
            if 3 <= num_files <= 5:
                ...
                if st.button("ตกลงและบันทึกข้อมูลผลการตรวจสอบ 🚀", type="primary", use_container_width=True):
                    with st.spinner('ระบบกำลังเชื่อมโยงและบันทึกรายงานผล...'):
                        st.balloons()
                        st.success(f"🎉 รายงานของ {u['ชื่อนามสกุล']} ตรวจเช็คเครื่องจักร {row['Asset No.']} บันทึกเข้าคลังข้อมูลสำเร็จ!")
                        time.sleep(3)
                        refresh_app()
            else:
                st.warning("⚠️ โปรดเปิดกล้องและส่งรูปถ่ายสภาพหน้างานจริงอย่างน้อย 3 รูปขึ้นไป เพื่อปลดล็อกปุ่มบันทึก")
