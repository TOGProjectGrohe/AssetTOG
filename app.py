...
# 🌐 ลิงก์ Web App URL ของคุณวีรพันธ์
    ...
    if input_id.strip() != "":
        ...
        if result["status"] == "success" and result.get("found"):
            ...
            if st.button("🔓 กดเพื่อเข้าระบบ"):
                # ฝังตัวแปรเก็บไว้ใน Session ให้เหนียวแน่น
                st.session_state.user_info = {"id": str(result["id"]), "name": str(result["name"]), "position": str(result["position"])}
...
# ---------------- หน้าสาม: บอร์ดสถิติอิง Material จริง ----------------
    ...
    # 💾 ปุ่มบันทึกข้อมูล (ซ่อมแซมกระบวนการล็อกตัวแปร String สมบูรณ์)
    if st.button("💾 บันทึกข้อมูล", key=f"save_btn_{defect}"):
        if not after_text.strip():
            ...
            # ควานหาดึงข้อมูลผู้ใช้ออกมาล็อกเป็นค่า String
            ...
            # บังคับดักจับ Position ไม่ยอมให้ว่างเปล่าเด็ดขาด ถ้าไม่มีเซ็ต "GL" ส่งออก
            ...
            # ถอดรหัสตัวเลข Defect
            clean_defect = "".join(filter(str.isdigit, str(defect)))
            val_errortype = str(clean_defect).strip() if clean_defect else str(defect)
            
            # ถอดตัวอักษรหน้างาน เช่น หน้า A -> A
            ...
            # สรุปโครงสร้างชุดข้อมูลส่งไปยัง Google Web App
            payload = {
                "timestamp": save_timestamp,
                "employee_id": emp_id,
                "employee_name": emp_name,
                "position": emp_position,                  # ส่งคีย์แมตช์ตรงกับ Apps script
                "material": val_material,
                "errortype": val_errortype,                # ส่งคีย์แมตช์ตรงกับ Apps script
                "improvement_type": val_improvement_type,  # ส่งคีย์แมตช์ตรงกับ Apps script
                "improvement_details": val_details         # ส่งคีย์แมตช์ตรงกับ Apps script
            }
            
            try:
                response = requests.post(APPS_SCRIPT_URL, data=json.dumps(payload), headers={"Content-Type": "application/json"})
                if response.status_code == 200:
                    st.success(f"🎉 บันทึกข้อมูลสำเร็จ! คอลัมน์ D ({emp_position}) และทุกช่องบันทึกลงตารางครบถ้วนแล้ว")
                    st.session_state.clear_trigger += 1
                    st.rerun()
                else:
                    st.error(f"❌ บันทึกไม่สำเร็จ (Error Code: {response.status_code}) โปรดเช็กการอัปเดตเวอร์ชันบน Apps Script")
